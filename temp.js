...
function createListItem(siteUrl, listName, itemProperties, success, failure) {
  ...
  // Đổ các quyền ra thẻ select-permission-edit
  // Chọn các giá trị ở trường visibleOnly
  ...
  // Đổ giá trị ra thẻ select-parent
  ...
  // Đổ giá trị ra thẻ select-parent
  // Chọn giá trị theo trường parent
  ...
  var ApiList = (function () {
    return {
        // Hàm dùng để tạo 1 phần tử trong list sharepoint
        createListItem: function (siteUrl, listName, itemProperties, success, failure) {
            var url = `${siteUrl}/_vti_bin/listdata.svc/${listName}`;
            $.ajax({
                url: url,
                type: "POST",
        ...
        // Lấy 1 phần tử trong list sharepoint
        getListItemById: function (siteUrl, listName, itemId, success, failure) {
            var url = `${siteUrl}/_vti_bin/listdata.svc/${listName}(${itemId})`;
            $.ajax({
                url: url,
                method: "GET",
        ...
        // Sửa 1 phần tử trong list sharepoint
        updateListItem: function (siteUrl, listName, itemId, itemProperties, success, failure) {
            getListItemById(
                siteUrl,
                listName,
                itemId,
        ...
        // Xóa 1 phần tử trong list sharepoint
        deleteListItem: function (siteUrl, listName, itemId, success, failure) {
            getListItemById(
                siteUrl,
                listName,
                itemId,
